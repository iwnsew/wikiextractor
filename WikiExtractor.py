...
#
acceptedNamespaces = set(['w', 'Category'])
...
def WikiDocument(out, id, title, redirect, text):
    url = get_url(id, prefix)
    header = ''
    if redirect != '':
        header = '<doc id="%s" url="%s" title="%s" redirect="%s">\n' % (id, url, title, redirect)
    else: 
...
def compact(text):
    ...
    for line in text.split('\n'):
        ...
        # handle lists
            ...
            if keepSections:
                beg = 1
                while beg < len(line) and line[beg] in '*#:;':
                    beg += 1
                #page.append("<li>%s</li>" % line[beg:])
                page.append(line[beg:])
            else:
                continue
    ...
    for line in input:
        ...
        if tag == 'page':
            ...
            if (colon < 0 or title[:colon] in acceptedNamespaces):# and not redirect:
                ...
                sys.stdout.flush()
                WikiDocument(output, id, title, redirect, ''.join(page))
            id = None
            page = []
        elif tag == 'base':
